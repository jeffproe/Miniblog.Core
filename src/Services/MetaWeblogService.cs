...
namespace Miniblog.Core.Services
{
	public class MetaWeblogService : IMetaWeblogProvider
	{
		...
		public Task<string> AddPostAsync(string blogid, string username, string password, Post post, bool publish)
		{
			...
			return Task.Run(() =>
			{
				var newPost = new Models.PostVM
				{
					Title = post.title,
					Slug = !string.IsNullOrWhiteSpace(post.wp_slug) ? post.wp_slug : Models.PostVM.CreateSlug(post.title),
		...
		private WilderMinds.MetaWeblog.Post ToMetaWebLogPost(Models.PostVM post)
		{
			var request = _context.HttpContext.Request;
			string url = request.Scheme + "://" + request.Host;
